...
Z <- scale(Z)
# Step4: 
# Fit the bivariate bkmr using fit_bkmr function
set.seed(123)
 fit_bkmr <- kmbayes(y = y, Z = Z, X = X, family = "binomial", 
                    iter = 5000, verbose = FALSE, varsel=TRUE)
# Step5:
# Compute the univariate exposure–response functions for each predictor across a grid of values, while holding other predictors at their median.
